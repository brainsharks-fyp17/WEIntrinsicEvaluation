...
# We use this script to evaluate word2vec model in terms of relatedness task
...
f_w = codecs.open("./results_relatedness_word2vec_300.txt", 'w', encoding='utf-8')

# load model
model = word2vec.Word2Vec.load("../NoiseRemoval/trained_fasttext_300/fasttext_300.w2v")
...
def evaluate_word_pairs(pairs, delimiter='\t'):
        ...
        for line in f:
            ...
            if (a not in model.wv.vocab) or (b not in model.wv.vocab):
                ...
                similarity_model.append(
                    similarity(np.copy(model.wv[a]), np.copy(model.wv[b])))  # Similarity from the model
    ...
    f_w.write(
        'Pearson correlation coefficient against %s: %f with p-value %f ' + pairs + " " + str(pearson[0]) + " " + str(
            pearson[1]) + "\n")
    f_w.write('Spearman rank-order correlation coefficient against %s: %f with p-value %f ' + " " + pairs + " " + str(
        spearman[0]) + " " + str(spearman[1])
              + "\n")
...
evaluate_word_pairs("./WordSim/wordsim353_sinhala_final.txt", "\t")
